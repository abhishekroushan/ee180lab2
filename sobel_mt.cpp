...
void *runSobelMT(void *ptr)
{
 ...
 Mat src;
  ...
  while (1) {
    ...
    //do the multithreading with performing grayscale function
    //on half the image on thread 1 and the other half on thread 2
    pc_start(&perf_counters);
    if (myID == thread0_id) {
    ...
    //do the multithreading with performing sobel_calc function
    //on half the image on thread 1 and the other half on thread 2
    pc_start(&perf_counters);
     if (myID == thread0_id) {
